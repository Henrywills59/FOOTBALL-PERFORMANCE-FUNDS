  ...
  return (
    <PublicSection id="performance" eyebrow="Verified Performance" title="A public results preview without private model data.">
      <div className="performance-grid public-performance-grid">
        <article><span>Total verified selections</span><strong>{total ? String(total) : "No verified public results yet"}</strong></article>
        <article><span>Win rate</span><strong>{total ? `${Math.round((wins / total) * 100)}%` : "Pending verified results"}</strong></article>
        <article><span>Recent result trend</span><strong>{trend || "Awaiting first settled cycle"}</strong></article>
        <article><span>Last updated</span><strong>{formatPublicDate(experience?.generatedAt)}</strong></article>
      </div>
